...
class Firebase {
  ...
  getThumbnails = async (
    ...
    if (uid) {
      ...
      ref = this.post
        .where(`tag.${tag.replace(/#/, '')}`, '>', 0)
        .orderBy(`tag.${tag.replace(/#/, '')}`, 'desc')
        .limit(num);
    } else {
  ...
  getTags = async (keyword = null) => {
    ...
    const querySnapshot = await this.tag
        ...
        .then(async doc => {
          if (!doc.exists) {
            ...
            this.user
              ...
              .doc(`${item.pid}`)
              .delete();
            const querySnapshot = await this.notification
              .where('type', '==', 'like')
      ...
      await Promise.all(
        querySnapshot.docs.map(async doc => {
          if (doc.exists) {
            ...
            const fromUser = await notification.from
              .get()
              .then(res => res.data());
